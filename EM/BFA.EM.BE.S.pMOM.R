...
BFA.EM.BE.P.pMOM <- function(x,v,b=NULL,q=2,eps=0.0001,it=50,seed=4,scaling=FALSE,init=NULL,hyper=list(theta=0.5,aS = 1,bS = 1,l0=0.1,l1=100,at=1,bt=1,epsM=0.001),varianceBE=FALSE,intercept=FALSE){
  ...
  while ((count < it) & (change > eps )) {
    ...
    ##M-step
    M.old<-M
    M<-M.pMOM(x,Ez.x,Ezz.x,D,M.old,p.gamma,diag(psi))
    #Does it maximise?
    ...
    if (varianceBE==FALSE){
      psi<-Psi.pMOM(x,Ez.x,Ezz.x,M,D,aS,bS,p.gamma)
      psi_aux<-diag(psi)
    }else{
      for (i in 1:p_b){
        x_b<-x[c(1:n)[b[,i]==1],]
        ...
        psi_aux[,i]<-diag(Psi.pMOM(x_b,z_b,zz_b,M,D,aS,bS,p.gamma))
      }
