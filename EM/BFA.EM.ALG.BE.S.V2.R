...
Psi.new<-function(x,Ez.x,Ezz.x,M,D,a,b){
  n = nrow(x)
  q = ncol(M)
  p = nrow(M)
  #diag((1/(n+a+2))*diag(t(x)%*%x-t(x)%*%Ez.x%*%t(M)+a*b))
  diag((1/(n+a+2+q))*diag(t(x)%*%x-2*t(x)%*%Ez.x%*%t(M)+M%*%Ezz.x%*%t(M)+a*b+diag(apply(D*M^2,1,sum))))
  #diag((1/(n+a+2))*diag(t(x)%*%x-2*t(x)%*%Ez.x%*%t(M)+M%*%Ezz.x%*%t(M)+a*b))+(1/(p+a+2))*diag(apply(D*M^2,1,sum))
  #diag((1/(n+a+2))*diag(t(x)%*%x-t(x)%*%Ez.x%*%t(M)+a*b))+(1/(q+a+2))*diag(apply(D*M^2,1,sum))
}
...
logpriorFA <- function(M,psi,gtheta,gamma,D,hyper,varianceBE=FALSE,wb=1){
  ...
  l.M=sum(l.M)
  l.total=l.psi+l.gamma.gtheta+l.M
